...
from .misc import _check_inputs, _flat_to_shape, _mixed_norm, reshape_tensor, _all_callback_names, _all_adjoint_callback_names


class OdeintAdjointMethod(torch.autograd.Function):

    @staticmethod
    def forward(ctx, func, y0, t, x, rtol, atol, method, options, adjoint_rtol, adjoint_atol, adjoint_method,
                adjoint_options, t_requires_grad, shapes, *adjoint_params):
    ...
    def backward(ctx, *grad_y):
        with torch.no_grad():
            ...
            t, y, x, *adjoint_params= ctx.saved_tensors
            grad_y = grad_y[0]
...
def odeint_adjoint(func, y0, t, x, *, rtol=1e-7, atol=1e-9, method=None, options=None, shapes=None,
    ...
    sh, func, y0, t, rtol, atol, method, options, decreasing_time = _check_inputs(func, y0, t, rtol, atol, method, options, SOLVERS)

    # Handle the adjoint norm function.
    state_norm = options["norm"]
    handle_adjoint_norm_(adjoint_options, sh, state_norm)

    solution = OdeintAdjointMethod.apply(func, y0, t, x, rtol, atol, method, options, adjoint_rtol, adjoint_atol,
                                    adjoint_method, adjoint_options, t.requires_grad, shapes, *adjoint_params)
