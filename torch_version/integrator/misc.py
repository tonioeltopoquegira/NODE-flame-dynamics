...
class _TupleFunc(torch.nn.Module):
    ...
    def forward(self, t, y, x):
        f = self.base_func(t, _flat_to_shape(y, (), self.shapes), x)
...
class _ReverseFunc(torch.nn.Module):
    ...
    def forward(self, t, y, x):
        return self.mul * self.base_func(-t, y, x)
...
class _PerturbFunc(torch.nn.Module):
    ...
    def forward(self, t, y, x, *, perturb=Perturb.NONE):
        ...
        if perturb is Perturb.NEXT:
            ...
            # Do nothing.
            pass
        return self.base_func(t, y, x)

def get_param_shapes(model):
    """
    Function to get the shapes of all parameters in the model.
    
    Args:
    - model (nn.Module): The PyTorch model whose parameters' shapes are to be retrieved.
    
    Returns:
    - List of tuples: Each tuple represents the shape of a parameter.
    """
    param_shapes = [param.shape for param in model.parameters()]
    param_shapes = tuple(param_shapes)
    return param_shapes

def reshape_tensor(tensor, param_shapes):
    """
    Function to reshape a 1D tensor according to the specified parameter shapes.
    
    Args:
    - tensor (torch.Tensor): The 1D tensor to be reshaped.
    - param_shapes (List of tuples): The shapes to which the tensor needs to be reshaped.
    
    Returns:
    - List of torch.Tensors: Each tensor is reshaped to one of the parameter shapes.
    """
    tensors = []
    start_idx = 0
    ...
    for shape in param_shapes:
        ...
        tensor_chunk = tensor[start_idx:end_idx].view(shape)
...
def _check_inputs(func, y0, t, rtol, atol, method, options,  SOLVERS):
    ...
    shapes = None #get_param_shapes(func)
    #print(y0)
